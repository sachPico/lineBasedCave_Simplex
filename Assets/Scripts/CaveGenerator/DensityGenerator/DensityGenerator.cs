...
public class DensityGenerator : MonoBehaviour
{
    ...
    public bool showGizmos, showVoxelDensity, showGeneratedVertexInfo, showDensityBoundaryGizmos;
        ...
        for(int k=0; k<_voxel.numberOfGeneratedMeshObject.z; k++)
        {
            for(int j=0; j<_voxel.numberOfGeneratedMeshObject.y; j++)
            {
                for(int i=0; i<_voxel.numberOfGeneratedMeshObject.x; i++)
                {
                    ...
                    chunkOffset.x = i*_voxel.boundaryWorldPos.x;
                    chunkOffset.y = j*_voxel.boundaryWorldPos.y;
                    chunkOffset.z = k*_voxel.boundaryWorldPos.z;
                    CreateBuffers();
                    
                    _linePropBuffer.SetData(_lineGenerator.GetLineProps());
                    _nodePropBuffer.SetData(_lineGenerator.GetNodes());
                    _octaveOffsetBuffer.SetData(octaveOffset);
                    ...
                    _densityShader.SetBuffer(0,"octaveOffset", _octaveOffsetBuffer);
                    _densityShader.SetBool("isWorldSpaceNoise", isWorldSpaceNoise);
                    _densityShader.SetFloat("noiseWeight", noiseWeight);
                    _densityShader.SetFloat("noiseScale", noiseScale);
                    _densityShader.SetFloat("smoothingFactor", smoothingFactor);
                    _densityShader.SetVector("noiseOffset", noiseOffset);
                    _densityShader.SetVector("offset", chunkOffset);
                    _densityShader.SetFloat("persistence", persistence);
                    _densityShader.SetFloat("lacunarity", lacunarity);
                    timeStart = (float)System.DateTime.Now.Second + ((float)System.DateTime.Now.Millisecond/1000);
                    _densityShader.Dispatch(0,numThreadGroup.x,numThreadGroup.y,numThreadGroup.z);
                    //_densityShader.Dispatch(0,_voxel.numVoxel.x+1,_voxel.numVoxel.y+1,_voxel.numVoxel.z+1);
    ...
    public void CreateBuffers()
    {
        ...
        _octaveOffsetBuffer = _octaveOffsetBuffer = new ComputeBuffer(octaves, 12);
    ...
    public void OnValidate()
    {
        ...
        lacunarity = Mathf.Clamp(lacunarity, 0.0001f, int.MaxValue);
        persistence = Mathf.Clamp(persistence, 0.0001f, int.MaxValue);
        noiseWeight = Mathf.Clamp(noiseWeight, 0.0001f, int.MaxValue);
        if(autoUpdate)
        {
            GenerateDensity();
        }
